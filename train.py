...
class Closure:
    def __call__(self):
        ...
        loss = self.pde + li * self.ic + lb * self.bc + 0.5 * (mi * self.ic**2 + mb * self.bc**2)
        loss.backward()
        return loss
...
opt = torch.optim.LBFGS([u], tolerance_grad=0, tolerance_change=0, history_size=15)
li, lb = 0, 0
mi, mb = 1, 1
epsilon = 1e-16
mu_max = 10000
...
l2rel = np.linalg.norm(u - u_e) / np.linalg.norm(u_e)
print("l2rel: %.6e" % l2rel)
